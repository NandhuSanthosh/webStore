...
<div class="w3l-table-info" style="margin-left: -320px; overflow-x: auto;">
					  <h2>View Complaint </h2>
						...
						<tbody>
						  <?php
                                                  $qr="select  * from complaint order by Id desc";
                                                  $qrr=  mysqli_query($con,$qr);
                                                  $i=1;
                                                  while($r=mysqli_fetch_array($qrr))
                                                  {
                                                      $shop=$r['Shop'];
                                                       $qbn=  mysqli_query($con, "select *from shop where Id='$shop'");
                                            $r33=  mysqli_fetch_array($qbn);
                                                  ?>
                                                    <tr>
                                                          <form name="f1" method="post" action="">
                                                        <td><?php echo $i;?></td>
                                                          <td><?php echo $r33['Name'];?></td>
                                                        <td><?php echo $r['User_id'];?></td>
                                                        
                                                        <td><?php echo $r['Complaint'];?></td>
                                                        
                                                         <td><img src="<?php echo $r['Image'];?>" width="60" height="60"/></td>                              
                                                        <td>  <a href="deltestimony.php?id=<?php echo $r['Id'];?>"><input type="button" name="btn" value="delete" onclick="alert('are you sure want to delete??')"> </a></td>
